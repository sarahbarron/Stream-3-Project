...
def view_cart(request):
    ''' renders the entire contents of the cart '''
...
def add_to_cart(request, id):
    ...
    when you select the quantity and press add the
    product id & quantity are added to the cart dictionary '''

    # gets the quantity inputted by the customer
    quantity = int(request.POST.get('quantity'))
    # gets the product object or 404
    product = get_object_or_404(Product, pk=id)
    # we get a cart that exists or an empty cart
    # if one is not already created
    cart = request.session.get('cart', {})
    # if the product is already in the cart
    # add the quantity in the cart to the new
    # customer inputted quantity
    if id in cart:
    ...
    if is_stock_available < 0:
        ...
        # send a message to the customer to tell
        # them the maximum available
        messages.info(request,
                      '%s is the maximum quantity we have at '
                      'this time we have amended your cart to '
                      'the maximum available' % quantity,
                      extra_tags="safe")
...
def adjust_cart(request, id):
    ...
    to the specified amount gets the existing
    ...
    if is_stock_available < 0:
        ...
        # of the stock levels and amendment
        messages.info(request,
                      '%s is the maximum quantity we have at this '
                      'time we have amended your cart to the maximum '
                      'available' % quantity,
                      extra_tags="safe")
    # we get a cart that exists or and empty
    # 1 if one is not already created
    cart = request.session.get('cart', {})
    # we can only adjust if there is something in the cart
    # therefore quantity must be greater that 0
    if quantity > 0:
        cart[id] = quantity
    # otherwise the quantity is 0 or less therefore
