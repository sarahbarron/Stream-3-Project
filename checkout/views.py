...
def checkout(request):
    ...
    if is_stock_available is True:
        return checkout_pay(request)
    # else if there is not enough stock continue to amend
    # the cart to available stock levels.
    elif is_stock_available is False:
...
def out_of_stock(request):
    ...
    for id, quantity in list(cart.items()):
        ...
        # from the database subtracts the quantity
...
def checkout_pay(request):
    ...
    if request.method == "POST":
        ...
        if order_form.is_valid() and payment_form.is_valid():
            ...
            total = 0
            # for each item in the cart calculate the total
            # and add to order line item
            for id, quantity in cart.items():
                # assign the logged in user to user
                user = request.user
                # get an instance of the product object or 404
                product = get_object_or_404(Product, pk=id)
                # total price equals the quantity multiplied
                # by the product price
                total += quantity * product.price
                # assigns order_line_item to OrderLineItem
                # which stores the order, product, quantity & user
    ...
    # return to checkout.html with the order form,
    # payment form & STRIPE.PUBLISHABLE key
