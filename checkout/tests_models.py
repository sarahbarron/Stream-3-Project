...
class TestOrderModel(TestCase):
    ...
    def test_order_model(self):
        ...
        # create an order
        order = Order(full_name='fullname',
                      phone_number='12345',
                      street_address1='first',
                      street_address2='last',
                      town_or_city='waterford',
                      country='Ireland',
                      postcode='eircode',
                      date="2018-06-01")
        # save the order
    ...
    def test_order_Line_Item_model(self):
        ...
        # save the user
        user.save()
        # create an order line item with the user,
        # product, order, quantity and status
        order_line_item = OrderLineItem(user=user,
                                        product=item,
                                        order=order,
                                        quantity="1",
                                        status="Delivered")
        # check the order_line_item fields are equal to
        # the values of order_line_item
    ...
    def test_return_order_id_date_and_fullname_as_a_string(self):
        ''' test return value for order is the expected string '''
    ...
    def test_OrderLineItem_return_string_qty_product_name_and_price(self):
        ''' test return value for order line item is the expected string '''
        # create a product
