...
def new_review(request, id, pk=None):
    ''' create a new reviw - user must be logged in '''

    # get an instance of the product being reviewed
        ...
        if form.is_valid():
            ...
            average = (average +
                       ((review_rating - average) /
                        new_num_of_ratings))
            # assign the products number_of_ratings
            # to the new number of ratings
            product.num_of_ratings = new_num_of_ratings
            # assign the products average rating
            # to the newly calculated ratings
    ...
    if review.user == request.user:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                # save the form
                review = form.save()
                # if the rating has changed to the original review rating
                # recalculate the average
                if review.rating != original_review_rating:
                    # calculating the average rating of the product
                    # subtract the old rating, add the new rating
                    ...
                    average = product.average_rating
                    # if the number of ratings equals 1 then
                    # this is the only review
                    # so the average is equal to this reviews rating
                    if num_of_ratings == 1:
                        average = review_rating
                    # otherwise
                    else:
                        # subtract the old rating and recalcualte the average
                        average = (((average * num_of_ratings) -
            ...
            # otherwise get an instance of the review form
            # and show it in the reviewform.html page
            form = ReviewForm(instance=review)
            return render(request, 'reviewform.html', {'form': form})
    else:
        # information message to be shown to the user
        messages.info(request, 'You are not authorised to edit this review')
        # if the logged in user is not the user who wrote the review
        # they can not edit the review and will be returned to the index page
...
def delete_review(request, pk):
    ...
    # get an instance of the review
    review = get_object_or_404(Review, pk=pk)
    # you can only delete a review if you are
    # the person who created the review or you are a staff member
    if review.user == request.user or request.user.is_staff:
        ...
        average = product.average_rating
        # if the new number of ratings is equal to 0
        # then the product has no reviews so the average is equal to 0
        if new_num_of_ratings == 0:
            average = 0
        # otherwise recalculate the products average
        # rating after the deletion of this review rating
        else:
            average = (((average * num_of_ratings) - review_rating)
                       / (new_num_of_ratings))
        # assign the new number of ratings for the product
        product.num_of_ratings = new_num_of_ratings
        # assign the new average rating for the product
        product.average_rating = average
        # save the new average to the product database
        product.save()
        # delete the review
        review.delete()
        ...
        # information message to be shown to the user
        messages.info(request, 'You are not authorised to delete '
                      'this review')
        # If the user trying to delete the review is not the user
        # who created the review do not let them delete the review
