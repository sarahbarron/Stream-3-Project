...
def register_customer(request):
    ...
    if request.user.is_authenticated:
        messages.info(request,
                      ...
                      '<a href="https://stream-3-'
                      'project-sarahbarron.c9users.io'
                      '/accounts/logout/">logout</a> '
                      'and click register again',
                      extra_tags="safe")
        return redirect(reverse('index'))
    # if its a POST method
    if request.method == "POST":
        # pass the inputted values into the form
        ...
        # to tell the person they can't register
        else:
            messages.error
...
def login_customer(request):
    ...
    # they can't login again so
    # return them the home page with a message
    # you are already logged in
    if request.user.is_authenticated:
        messages.info(request,
                      'You are already logged in! If this '
                      'is not you <a href="https://stream-3'
                      '-project-sarahbarron.c9users.io/'
                      'accounts/logout/">logout</a> and '
                      'log back in again under your own username',
                      extra_tags="safe")
        return redirect(reverse('index'))
    # if its a POST method
    if request.method == "POST":
        # pass in the values that the customer has
        # inputted for username and password
        customer_login_form = CustomerLoginForm(request.POST)
        # check to see if the data in the
        # customer login form is valid
        if customer_login_form.is_valid():
            ...
            if customer:
                ...
                # if the customer is required to login
                # for checkout / reviews they will be
                # redirected back to the checkout or
                # review pages after login
                nexturl = request.POST.get('next')
                if nexturl:
                    return redirect(nexturl)
                # otherwise redirected back to the index.html page
                else:
                    return redirect('index')
            # otherwise send a message to say the form is invalid
            else:
                customer_login_form.add_error(None,
                                              'Your username or password is'
                                              ' incorrect, please try again!')
    # If there is no POST method return an empty
    # instance of the customer login form
    else:
...
# the logout_customer view can only be accessed
# if the customer is already logged in
@login_required
def logout_customer(request):
    ''' logout a logged in customerr '''
...
def customer_profile(request):
    ''' a logged in customers profile '''

    # an instance of the customer that is logged in
    profileuser = request.user
    # an instance of all orders made by logged in user
    # and order them by most recent date
    myorders = OrderLineItem.objects.filter(
        user=profileuser).order_by('-order')
    ...
    try:
        ...
        # give the last page of results.
        orders = paginator.page(paginator.num_pages)
    # an instance of all reviews made by the customer
    # logged in and order it by most recent date
    customer_review = Review.objects.filter(
        user=profileuser).order_by('-date')
    ...
    # return the user matching the email address stored in the request
